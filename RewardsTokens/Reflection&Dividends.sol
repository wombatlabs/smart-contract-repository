...
FEATURES
* Reflection + Dividend rewards
* Sell and hold limits
...
contract XXXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("XXX_Dividend_Tracker", "XXX_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 100+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "XXX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "XXX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CMAC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "XXX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "XXX_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TOKENNAME is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    XXXDividendTracker public dividendTracker;    

    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingAddress = payable(0x81c070339452455a8840b98C7e822BB3a4c5BBbD);

    uint256 public maxTransactionAmount = 1 * 10 ** 6 * (10**18);
    uint256 public maxDailySellAmount = 1 * 10 ** 6 * (10**18);
    ...
    constructor()  {
    	...
    	dividendTracker = new XXXDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        XXXDividendTracker newDividendTracker = XXXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "XXX: The new dividend tracker must be owned by the Verse token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XXX: Account is already 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XXX: The UniSwap pair cannot be removed from automatedMarketMakerPairs");
